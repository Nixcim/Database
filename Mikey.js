...
setInterval(function(){
  ...
  let sikiş = new MessageEmbed().setColor("RANDOM").setAuthor("")

  client.guilds.cache.get(ayarlar.guildID).channels.cache.get(channel).send(sikiş.setDescription(`Sunucu Urlsi \`\`${ayarlar.sunucuURL}\`\` olarak Veri Tabanina Kayıt Edildi.`))
...
client.on("message", async message => {
  ...
  if (command === "eval" && message.author.id === ayarlar.botOwner) {
    ...
    if (!args[0]) return message.channel.send(`Kodunu belirt`);
  ...
  if(command === "ryükle") {
    ...
    if (!args[0] || isNaN(args[0])) return message.channel.send(embed.setDescription("Rol Veri Tabanında Olan Rol ID Seç."));



    Database.findOne({guildID: ayarlar.guildID, roleID: args[0]}, async (err, roleData) => {

      if (!roleData) return message.channel.send(embed.setDescription("Belirtin ID Rol Verisi Bulunamadı."));
      ...
      let yeniRol = await message.guild.roles.create({
        ...
        reason: "`Rol Verisine Göre Rol Yeniden Açıldı.`"
  ...
  if (!args[0] || isNaN(args[0])) return message.channel.send(`Veri Tabanında Olan Geçerli bir Kanal ID'si belirtmelisin.`);
  ...
  ChannelData.findOne({guildID: ayarlar.guildID, channelID: args[0]}, async (err, channelData) => {
    ...
    .setDescription(`**${channelData.name}** isimli kanalın backup Verisi  kullanılarak Sunucda Sekronize Edilecektir.`)
    ...
    await message.channel.send({ embed: kEmbed }).then(msg => {
      ...
      collect.on("collect", async r => {
        ...
        setTimeout(async function(){
          ...
          msg.delete().catch(err => console.log(`Veri tabanından Backup Mesajı Verisi Silinemedi.`));
          ...
          if (!savedChannel) {
            ...
            if(channel.type === "voice"){
              ...
              let newChannelSchema = new ChannelData({
                ...
                topic: channel.topic ? channel.topic : "Bu kanal Backup Veri Tababnına Göre Yenide Oluşturuldu!",
...
message.channel.send("Güvenlik Amacıyla Rol verileri başarı ile kayıt edildi.")

message.channel.send("Güvenlik Amacıyla Kanal verileri başarı ile kayıt edildi.")

console.log("Güvenlik Amacıyla Bütün kanal ve rol verileri başarı ile kayıt edildi.")
...
function otokayıt (guildID) {
 ...
 verikanalı.send(embed.setDescription("✅ Güvenlik Amacıyla Rol verileri başarı ile kayıt edildi. \n✅ Güvenlik Amacıyla Kanal verileri başarı ile kayıt edildi."))
      ...
      ChannelData.findOne({guildID: ayarlar.guildID, channelID: channel.id}, async (err, savedChannel) => {
        ...
        if (!savedChannel) {
          ...
          if(channel.type === "voice"){
            ...
            savedChannel.topic = channel.topic ? channel.topic : "Bu kanal Backup Veri Tababnına Göre Yenide Oluşturuldu!";
